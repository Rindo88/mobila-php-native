...
<head>
  ...
  <title>Mobila - Platform Jual Beli Mobil Terpercaya</title>
  <!-- Tailwind CSS -->
  ...
  <style>
    ...
    /* Animasi fade out untuk preloader */
    @keyframes fadeOut {
      from {
        opacity: 1;
      }
...
<body class="bg-gray-50 text-gray-900 font-sans loading">

  <!-- PRELOADER -->
  <div id="preloader" class="fixed inset-0 z-[9999] flex items-center justify-center bg-gradient-to-br from-red-500 via-red-600 to-red-800">
    ...
    <div class="relative text-center px-4">
      ...
      <div class="mb-8 animate-bounce">
        <img src="./assets/img/logomobila.png" alt="Mobila Logo" class="h-20 w-auto mx-auto drop-shadow-2xl" />
      </div>

      <!-- Car Animation -->
      <div class="relative h-24 mb-8 overflow-hidden">
        <div class="car-loader absolute bottom-0">
          <svg width="120" height="60" viewBox="0 0 120 60" fill="none" xmlns="http://www.w3.org/2000/svg">
            <!-- Car Body -->
            <path d="M20 30 L30 20 L70 20 L80 30 L100 30 L100 45 L15 45 L15 30 Z" fill="white" stroke="white" stroke-width="2"/>
            <rect x="35" y="22" width="15" height="10" fill="#EF4444" rx="2"/>
            <rect x="55" y="22" width="15" height="10" fill="#EF4444" rx="2"/>
            <!-- Wheels -->
            <circle cx="30" cy="50" r="8" fill="#1F2937" class="wheel"/>
            <circle cx="30" cy="50" r="4" fill="#6B7280"/>
            <circle cx="85" cy="50" r="8" fill="#1F2937" class="wheel"/>
      ...
      <div class="flex items-center justify-center gap-2 mb-6">
        <h2 class="text-white text-2xl font-bold tracking-wide">Memuat</h2>
      ...
      <!-- Progress Bar -->
      <div class="w-80 max-w-full h-2 bg-white bg-opacity-20 rounded-full overflow-hidden mx-auto backdrop-blur-sm">
        <div id="progressBar" class="h-full bg-white rounded-full shadow-lg progress-animate"></div>
      </div>

      <!-- Loading Message -->
      <p class="text-white text-sm mt-6 opacity-90 font-medium">Menyiapkan platform terbaik untuk Anda...</p>
    </div>
  </div>
  ...
  <div id="mainContent" class="hidden">
          ...
          if(mysqli_num_rows($carResult) > 0) {
            while($car = mysqli_fetch_assoc($carResult)) {
              ...
              if(mysqli_num_rows($imgResult) > 0) {
                ...
                $imagePath = 'uploads/' . $imgRow['gambar'];
              } else {
                $imagePath = 'uplouds/';
              }
              ?>
    ...
    <section id="ulasan" class="py-16 bg-white">
      <div class="container mx-auto px-4">
        ...
        <div class="text-center mt-10">
          <a href="login.php" class="inline-block border border-red-600 text-red-600 font-semibold px-6 py-2 rounded hover:bg-red-600 hover:text-white transition">Lihat Semua Ulasan</a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          if(mysqli_num_rows($newsResult) > 0) {
            while($news = mysqli_fetch_assoc($newsResult)) {
              $imagePath = !empty($news['gambar']) ? 'uploads/' . $news['gambar'] : 'uploads/';
    ...
    <footer class="bg-gray-900 text-gray-300 py-10">
      ...
      <div class="text-center text-gray-500 mt-8 text-sm">
        &copy; <?= date('Y') ?> Mobila. Dibuat dengan oleh Kelompok 5
      </div>
    </footer>
  </div>
  ...
  <script>
    ...
    window.addEventListener('load', () => {
      ...
      setTimeout(() => {
        // Tambahkan animasi fade-out ke preloader
        preloader.classList.add('fade-out');
        
        // Setelah animasi selesai, sembunyikan preloader dan tampilkan konten
        setTimeout(() => {
          preloader.style.display = 'none';
          mainContent.classList.remove('hidden');
          mainContent.classList.add('fade-in-up');
          document.body.classList.remove('loading');
        }, 1000); // Sesuai dengan durasi animasi fadeOut
